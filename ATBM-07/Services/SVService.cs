...
namespace ATBM_07.Services
{
    public static class SVService
    {
        public static DataRow GetCurrentSinhVien_SV()
        {
            ...
            using (var cmd = new OracleCommand("SELECT * FROM user_admin.SINHVIEN", conn))
        ...
        {
            using (var conn = DatabaseHelper.GetNewConnection())
            using (var cmd = new OracleCommand("user_admin.UPDATE_SINHVIEN_SV", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            using (var conn = DatabaseHelper.GetNewConnection())
            using (var cmd = new OracleCommand("user_admin.ADD_DANGKY_SV", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            using (var conn = DatabaseHelper.GetNewConnection())
            using (var cmd = new OracleCommand("user_admin.DELETE_DANGKY_SV", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("p_mamm", OracleDbType.Varchar2).Value = mamm;
                cmd.ExecuteNonQuery(); // Vì không có kết quả trả về
        ...
        {
            using (var conn = DatabaseHelper.GetNewConnection())
            using (var cmd = new OracleCommand("user_admin.UPDATE_DANGKY_SV", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
